...
def Feature_GCN(X):
    """
    Group feature may have a cluster with only one element, where corr - eye returns a zero matrix; so it's handled to return 1 when set to 0 here.
    """
...
def generate_next_state_of_meta_cluster1(X, y, dqn_cluster, cluster_num=2,
    ...
    for key, value in clusters.items():
        ...
        cluster_list.append(key)
        action_list.append(action)
    #    # 1. Use argmax directly here; previously it was epsilon-greedy + argmax
